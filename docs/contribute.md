...
└── site/                  # Hugo documentation website (optional)
```
...
- Check [ADRs](decisions/) to understand the design philosophy
- If it's an analyzer, see [analyzer interface](concepts/analyzer-interface.md)
...
2. Create an ADR if it affects architecture (see [template](decisions/0000-template.md))
...
- **Architecture docs:** See [docs/](.)
- **API docs:** Update [api/crds.md](api/crds.md) if you change CRDs
- **README files:** Update component READMEs in [components/](components/)

### 5. Analyzers

Analyzers are deterministic rule-based checks. To add one:

1. Implement the [analyzer interface](concepts/analyzer-interface.md)
2. Add tests (use real xDS data from istioctl or KIND)
3. Document the check in [analyzer-catalog.md](api/analyzer-catalog.md)
4. Wire it into the server's analyzer catalog
...
- **Design:** Does it follow [ADR principles](decisions/)?
