...
| Evidence enrichment | Deterministic table (`internal/server/enrich`) mapping a finding's analyzer to a fixed set of MCP tool calls, run before every LLM call — richer evidence without the LLM ever choosing what to fetch ([ADR-0006](../../decisions/0006-mcp-evidence-enrichment.md), [`../mesh-mcp/README.md`](../mesh-mcp/README.md)). |
| LLM gateway | Turns a Finding/Incident + its (enriched) evidence into a prompt, calls the configured model, and validates the structured response before storing it. Two calls per finding — explain, then propose — never one ([ADR-0002](../../decisions/0002-deterministic-analyzers-then-llm.md)). |
